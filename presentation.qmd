...
2.  Goals for the project - correctly identify people with Alzheimers
3.  Brief dataset description - types of data

# Key Findings

1.  Announce the best model - simple terms ?? Combination of Lasso + Random Forest??
2.  Sensitivity a key performance measure. Explain why?
3.  ::Visual::

# Our Approach

1.  To find the best performing model, we tested 4 models
2.  Phrase everything relatively
3.  kNN, LDA, RF, Lasso - share simple analogies
4.  Sensitivity comparison visual for the 4 models on the test data. Underline the significance for choosing senstivity
5.  We reached our goal (refering to the problem)
...
predicted_labels <- ifelse(predictiontree[, "Yes"] > 0.5, "Yes", "No")
...
TP1 <- confusion_matrix_singletree$table["Yes", "Yes"]
FP1 <- confusion_matrix_singletree$table["Yes", "No"]
FN1 <- confusion_matrix_singletree$table["No", "Yes"]
precision1 <- TP1 / (TP1 + FP1)
recall1 <- TP1 / (TP1 + FN1)
F1_1 <- 2 * (precision1 * recall1) / (precision1 + recall1)
```

**Sensitivity:** `r round(Sensit1, 4)` **Specificity:** `r round(Specif1, 4)` **Accuracy:** `r round(Accura1, 4)`<br> **F1:** `r round(F1_1, 4)`
:::
:::
